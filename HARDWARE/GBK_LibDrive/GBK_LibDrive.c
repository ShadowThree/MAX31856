...
//	
//		RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOC, ENABLE);//使能 PB,PC 时钟
...
//		
//		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_15;      //设置为推挽输出
//		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;  //输出模式
//	  GPIO_Init(GPIOB, &GPIO_InitStructure);         //初始化
//		
//		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;     //设置为推挽输出
//		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;  //输出模式
...
//支持自动换行
// (x,y)        :起始坐标
// width,height :可以显示字符的区域(这里的区域必须大于等于size)
// str          :字符串
// size         :字体大小
// D_Color      ：点阵字颜色      --16位
// B_Color      ：背景颜色        --16位
		...
		if(!bHz)
    {
        ...
        if(x>(x0+width-size/2))//换行
				...
				{         
					...
					GBK_ShowASCII(x, y, str, size, D_Color, B_Color, mode);  //LCD_ShowChar(x,y,*str,size,mode);//有效部分写入 
...
// x y 			: 指定显示区域的左上角坐标
// size 		: 指定字体大小，只能取12，24，36，48，64等值
// len 			: 指定要显示的宽度
// D_Color 	：点阵字颜色  --16位
// B_Color 	：背景颜色    --16位
//********************************************************************************
void GBK_Show_StrMid(uint16_t x,uint16_t y, uint8_t size, uint8_t len, uint16_t D_Color, uint16_t B_Color,uint8_t*str)
{
  ...
  strlenth = strlen((const char*)str);
	strlenth *= size/2;
	if(strlenth>len)
		GBK_Show_Str(x,y,lcddev.width,lcddev.height,str,size,D_Color,B_Color,1);
	else
	{
